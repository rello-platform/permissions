...
export const PERMISSIONS = {
  ...
  // ─── Rate-sheet ingestion mutations cross-app write (Rello → PFP operator actions) ──
  // Inverse-verb sibling to RATE_SHEETS_READ. Per the upcoming PFP rate-
  // ingestion operator-actions wave: Rello's Platform Admin "Interest Rates"
  // management surface fires operator mutations against PFP's canonical
  // AgentRateSheet ingestion pipeline (PFP is canonical owner per APP-OWNERSHIP-
  // MATRIX + D1 lock 2026-05-13). These are state-changing operator actions
  // (re-fetch a lender sheet, reset an auto-disable, repoint a sheet URL, retry
  // a dead-lettered extraction, force a snapshot refresh) — they require a
  // dedicated write permission distinct from the read-only RATE_SHEETS_READ
  // serve path so the operator-mutation surface can be granted/revoked
  // independently.
  //
  // Surface: PFP POST/PATCH /api/tenants/[tenantId]/rate-sheets/* operator
  // mutation routes. Validated by PFP's requireServiceBearer (Path A: Bearer
  // rello_*; SHA-256 hash match advances ApiKey.lastUsedAt; per-pair
  // appSource/targetApp isolation enforced — appSource=RELLO,
  // targetApp=PATHFINDER_PRO). Topology #3 (Spoke-validated, Rello-granted) per
  // API-KEY-LIFECYCLE-README §2 — sibling to RATE_SHEETS_READ. Additive only —
  // no slug removed or renamed.
  RATE_SHEETS_WRITE: {
    slug: "rate-sheets:write",
    label: "Write platform rate-sheet ingestion mutations",
    description: "Rello → Pathfinder Pro POST/PATCH /api/tenants/[tenantId]/rate-sheets/* operator-action mutation routes per-caller credential. Used by Rello's Platform Admin Interest Rates management surface to fire state-changing rate-sheet ingestion operator actions against PFP's canonical AgentRateSheet pipeline (re-fetch a lender sheet / reset an auto-disable / repoint a sheet URL / retry a dead-lettered extraction / force a snapshot refresh). PFP is canonical owner of AgentRateSheet ingestion per APP-OWNERSHIP-MATRIX:321 + D1 lock 2026-05-13. Distinct from the read-only rate-sheets:read serve path so the operator-mutation surface can be granted/revoked independently. Receiver validates via requireServiceBearer (Path A: Bearer rello_*). Topology #3 (Spoke-validated, Rello-granted) per API-KEY-LIFECYCLE-README §2 — inverse-verb sibling to rate-sheets:read.",
    validatedBy: ["pathfinder-pro"],
    grantedTo: ["rello"],
  },
